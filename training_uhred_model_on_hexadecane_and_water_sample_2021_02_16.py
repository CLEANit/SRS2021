# -*- coding: utf-8 -*-
"""Training UHRED model on Hexadecane and water sample 2021-02-16.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1e1dNlDL-Ls_hF3qdyj1WpTJtBltBoVzE
...
"""Input data is a noisy hyperspectral SRS image."""

#Importing data, store it in to a numpy array (noisy_data) and then print the shape of it. 
input_data = read_tiff("/content/drive/My Drive/Data/2020-03-05 oil and water /Scan 2/20 mW/20 mW.tif") 
...
#Importing data, store it in to a numpy array (noisy_data) and then print the shape of it.
GT = read_tiff("/content/drive/MyDrive/Data/2020-03-05 oil and water /Scan 2/60 mw/60 mW.tif") 
#printing size of the data file
print(GT.shape)

"""**Comparison between the ground-truth and the noisy input dataset.**
...
im1 = ax1.imshow(GT[47,:,:],cmap = 'Spectral_r')
divider = make_axes_locatable(ax1)
cax = divider.append_axes("right", size="5%", pad=0.05)
fig.colorbar(im1, cax=cax, orientation='vertical')

im2 = ax2.imshow(input_data[47,:,:])
...
#Reading min, max and mean value of the imported data (Noisy input)
mean_input_N = np.mean(input_data)
max_input_N = np.max(input_data)
min_input_N = np.min(input_data)
...
Normalizing & reshaping the input dataset and ground truth."""
...
for i in range (256):
  for j in range (256):
    for k in range (92):
      ...
      N_noisy[i,j,k] = ((input_data[k,i,j]-mean_input_N))/(max_input_N-min_input_N)
...
"""Normalization of the noisy input dataset.
...
#Normalizing input dataset. To have mean value of 0 and standard deviation of 1. 
P_N_Noisy = (P_N_Noisy-np.mean(P_N_Noisy))/(np.max(P_N_Noisy)-np.min(P_N_Noisy))

#Ploting a all 92 images and save it in a file as img. "Processe noisy input"
A = P_N_Noisy.reshape(-1,92)
...
#Loading data on dataloader 
n_train = int(len(noisy_input_t)*0.70)
n_test = int(len(noisy_input_t)) - n_train
train_data_set, test_data_set = torch.utils.data.random_split(noisy_input_t, [n_train, n_test])
...
scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=290, gamma=0.7, last_epoch=-1)
...
for epoch in range(num_epochs):
  ...
  with torch.no_grad():
    ...
    if loss < best_error:
      ...
      model_save_name = 'UHRED model 2020-02-19'
